...
class Adapter(nn.Module):
    def __init__(self, in_dim, bottle_dim, adapter_scalar=0.1, scalar_learnable=False, adapter_type='parallel',
                 dtype=None):
        super().__init__()
...
class ViT_Tuner(nn.Module):
    def __init__(self, cfg, clip_model, text_features, teacher=True):
        super().__init__()
        ...
        if teacher:
            ...
            use_finetune = cfg.stu_finetune
            use_vpt_deep = cfg.stu_vpt_deep
            use_adapter = cfg.stu_adapter
            use_lora = cfg.stu_lora
            use_adaptformer = cfg.stu_adaptformer
            vpt_len = cfg.stu_vpt_len
        ...
        if use_vpt_shallow:
            ...
            vpt_list = nn.ModuleList([
                ...
                *[VPT(vpt_len=vpt_len, seq_len=seq_len, patch_size=patch_size, emb_dim=emb_dim, dtype=dtype) for _ in
                  range(partial)]
            ])
        elif use_vpt_last:
            vpt_list = nn.ModuleList([
        ...
        if use_adaptformer:
            adaptformer_list = nn.ModuleList([
                *[None] * (n_layers - partial),
                *[AdaptFormer(d_model=emb_dim, bottleneck=ffn_num, adapter_scalar=cfg.adapter_scalar,
                              scalar_learnable=cfg.scalar_learnable,
        ...
        if cfg.rand_init:
            head = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim).to(clip_model.dtype)
        else:
            head = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim, text_features=text_features).to(
                clip_model.dtype)

        if cfg.rand_init1:
            head1 = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim).to(clip_model.dtype)
        else:
            head1 = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim, text_features=text_features).to(
                clip_model.dtype)
...
class CLIP_ViT_INK(nn.Module):
    def __init__(self, clip_model):
        ...
        # self.proj = clip_model.visual.proj
        # self.dtype = clip_model.dtype
        self.patch_embedding = clip_model.patch_embed.proj
        self.class_embedding = clip_model.cls_token
        self.positional_embedding = clip_model.pos_embed
        self.ln_pre = clip_model.norm_pre
        self.blocks = clip_model.blocks
        self.ln_post = clip_model.norm
        self.proj = nn.Identity()
        self.out_dim = self.ln_post.bias.shape[0]
        self.dtype = clip_model.patch_embed.proj.weight.dtype
...
class ViT_INK_Tuner(nn.Module):
    def __init__(self, cfg, clip_model, text_features, teacher=True):
        super().__init__()
        n_layers = len(clip_model.blocks)
        emb_dim = clip_model.pos_embed.shape[2]
        seq_len = clip_model.pos_embed.shape[1]
        patch_size = clip_model.patch_embed.proj.kernel_size
        dtype = clip_model.patch_embed.proj.weight.dtype
        ...
        if use_finetune:
            if block_num is None:
                if nbackbone.startswith("IN21K"):
                    finetune_list = nn.ParameterList([
                        param for name, param in clip_model.named_parameters()
                    ])
                else:
                    finetune_list = nn.ParameterList([
                        param for name, param in clip_model.visual.named_parameters()
        ...
        # visual_proj = clip_model.visual.proj.data
        # logit_scale = clip_model.logit_scale.data
        visual_proj = torch.randn(2, 3)

        if cfg.rand_init:
            head = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim).to(dtype)
        else:
            head = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim, text_features=text_features).to(dtype)

        if cfg.rand_init1:
            head1 = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim).to(dtype)
        else:
            head1 = ViT_Head(cfg.DATA.NUMBER_CLASSES, visual_proj, emb_dim, text_features=text_features).to(dtype)
...
class RN_Head(nn.Module):
    ...
    def __init__(self, n_cls, logit_scale):
        super().__init__()
        # n_cls = text_features.shape[0]
        # self.weight = nn.Parameter(text_features.clone())
        self.weight = nn.Parameter(torch.empty(n_cls, 1024))
        self.weight.data.uniform_(-1, 1).renorm_(2, 0, 1e-5).mul_(1e5)
        # self.logit_scale = nn.Parameter(logit_scale.exp().clone())
        self.logit_scale = nn.Parameter(torch.ones([]) * (1 / 0.07))
...
class RN_Tuner(nn.Module):
    def __init__(self, cfg, clip_model, text_features):
        ...
        # use_finetune = cfg.finetune
        use_finetune = cfg.rn_finetune
        # use_bias_tuning = cfg.bias_tuning
        # use_bn_tuning = cfg.bn_tuning
        use_bias_tuning = cfg.rn_bias_tuning
        use_bn_tuning = cfg.rn_bn_tuning
        use_ssf = cfg.ssf
        ...
        if use_finetune:
            finetune_list = nn.ParameterList()
            if cfg.tuning_layer < 0:
                finetune_list = nn.ParameterList([
                    param for name, param in clip_model.visual.named_parameters()
                ])
            elif cfg.tuning_layer == 0:
                for name, param in clip_model.visual.named_parameters():
                    if name.startswith("attnpool"):
                        finetune_list.append(param)
            elif cfg.tuning_layer == 1:
                for name, param in clip_model.visual.named_parameters():
                    if name.startswith("attnpool") or name.startswith("layer4"):
                        finetune_list.append(param)
            # print('-')
        else:
            finetune_list = None
        ...
        # head1 = RN_Head(text_features, logit_scale).to(clip_model.dtype)
        head = RN_Head(cfg.DATA.NUMBER_CLASSES, logit_scale).to(clip_model.dtype)
        head1 = RN_Head(cfg.DATA.NUMBER_CLASSES, logit_scale).to(clip_model.dtype)
...
class Model(nn.Module):
    def __init__(self, cfg, clip_model, text_features, teacher=True):
        ...
        if teacher:
            ...
            backbone = cfg.stu_backbone
        # if cfg.backbone.startswith("ViT"):
        if backbone.startswith("ViT"):
            self.image_encoder = CLIP_ViT(clip_model)
            self.tuner = ViT_Tuner(cfg, clip_model, text_features, teacher=teacher)
        elif backbone.startswith("IN21K"):
            self.image_encoder = CLIP_ViT_INK(clip_model)
            self.tuner = ViT_INK_Tuner(cfg, clip_model, text_features, teacher=teacher)
        else:
