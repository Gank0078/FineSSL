...
#             temp_data_list.extend(self.cls_data_list[y][:self.n_max])
#         random.shuffle(temp_data_list)
        
#         for i in range(self.num_samples):
#             yield temp_data_list[i]
...
class DownSampler(Sampler):
    def __init__(self, data_source, n_max=100):
        ...
        self.cls_num_list = [min(n_max, len(x)) for x in cls_data_list]
        self.num_samples = sum(self.cls_num_list)
        
    def __iter__(self):
        temp_data_list = []
        for y in range(self.num_classes):
            for i in range(self.cls_num_list[y]):
                temp_data_list.append( next(self.data_iter_list[y]) )
        random.shuffle(temp_data_list)
